...
if($_POST['update'])
	{ 
		...
		if($nama=="" or $alamat=="" or $email=="" or $komentar=="")
		       {   
			$kosong=1; 
			$aktiv=1;
		       }
		elseif(!preg_match($pattern,$email))		
			...
			$noemail=1;		
		       ...
		       {
			   $query= "insert into formulir1 (nama,alamat,kota,provinsi,email,komentar) 
				    values ('$nama','$alamat','$kota','$provinsi','$email', '$komentar')";
				     mysql_query($query) or die (mysql_error() );
			   if($img_name<>"")
				{ 
				     copy ("$img","images/$img_name");
				     $query= "update formulir1 set img='$img_name' where email='$email'";
				      mysql_query($query) or die (mysql_error() );
				}			
